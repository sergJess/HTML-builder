...
async function createAssets(inputPath, outputPath){
  ...
  for await (let file of folder) {
    ...
    if(file.isFile()){
      const readFileStream = await fsFull.createReadStream(path.join(inputPath, file.name));
      const writeFileStream = await fsFull.createWriteStream(path.join(outputPath, file.name));
